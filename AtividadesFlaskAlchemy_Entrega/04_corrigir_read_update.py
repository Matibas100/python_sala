...
Execute:
  python 04_corrigir_read_update.py
"""
...
def listar_alunos():
    ...
    return Aluno.query.order_by(Aluno.nome).all()  # feito


def buscar_aluno(aluno_id):
    """READ - busca um aluno pelo id."""
    return db.session.get(Aluno, aluno_id)  # feito
...
if __name__ == "__main__":
    with app.app_context():
        if Aluno.query.count() == 0:
            db.session.add(Aluno(nome="Joao", email="joao@mail.com"))
        ...
        print("--- Update id=1 ---")
        ok = atualizar_aluno(1, "Joao Pedro", "joao.pedro@mail.com")
